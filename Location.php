...
if($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	$lng = $_POST["long"];
	$lname = $_POST["loname"];
	$locationQuery = "INSERT INTO Location(lid, lat, lng, lname) VALUES(16,'$lat', '$lng', '$lname')";
	...
	if ($searchResult ){
		...
		if(!is_null($row["lname"])){
			...
			if(mysqli_query($conn, $locationQuery)===TRUE){
				print '<script type="text/javascript">'; 
...
<head>
  ...
  <style>
    ...
    #input-info {position: absolute; top: 45px; left: 10px; right: 10px;}
    #submit-btn{width: 15%;}
    #map-canvas { position: absolute;top: 80px; bottom: 65px; left: 10px; right: 10px; }
    #map-output { position: absolute;bottom: 10px; left: 10px; right: 10px; }
    #map-output a { float:absolute;bottom: 10px; left: 10px; right: 10px; right; }
...
<body>
  ...
  <div id="input-info">
  	...
  	<input id="search-txt" type="text" placeholder="Name this new location" name="loname" maxlength="100">
  	<input id="submit-btn" type="submit" value="PIN this Location">
  	<input type="hidden" name="lati" >
  	<input type="hidden" name="long" >
  	</form>
  </div>
  <div id="map-canvas" style="height:600px;width:700px"></div>
  ...
  <script type="text/javascript">
    function loadmap() {
      ...
      google.maps.event.addListener(map, "idle", function() {
        ...
        document.myform.lati.value =  map.getCenter().lat().toFixed(3);
        document.myform.long.value =  map.getCenter().lng().toFixed(3); 
      });
